//! [`HookPoint`] — the lifecycle moments a module can subscribe to.
//!
//! This is the *vocabulary* half of the hooks system, and it lives in
//! `gt-module` for the same reason [`Scope`](crate::Scope) and
//! [`EventKind`](crate::EventKind) do: a module declares which points it observes
//! inside its [`Capability`](crate::Capability), and `gt-module` may not depend on
//! a sibling crate. `HookPoint` is plain `Copy` data with no async or `dyn`, so it
//! sits cleanly in the sync core (non-negotiable #2). The behavioral half —
//! `HookHandler`, `HookRegistry`, the dispatcher — lives in `gt-hooks`, which
//! depends on `gt-module` and re-exports this type so handler authors import it
//! from one place. Relocated here from `gt-hooks` by `hq-mod-hooks.3`.

use serde::{Deserialize, Serialize};

/// A point in an entity's lifecycle where modules may observe and react.
///
/// A module declares the points it subscribes to via
/// [`Capability::subscribing`](crate::Capability::subscribing); `gt-hooks`
/// registers a handler against the point and the dispatcher invokes it when the
/// kernel reaches that point. Exactly one point — [`BeforeCommand`](Self::BeforeCommand)
/// — is *vetoable*: a handler there may stop the operation via `HookOutcome::Reject`
/// before any effect occurs (the Sheriff pre-merge watchdog seam,
/// `hq-mod-hooks.7`). Every other point fires on a decided fact and observes only;
/// see [`is_vetoable`](Self::is_vetoable).
...
impl HookPoint {
    ...
    /// point that fires before any effect, so a `Reject` there can still stop the
    /// work. Every other point — `After*`, the event pair, and the bead-lifecycle
    /// points — observes a settled fact, and the dispatcher (`hq-mod-hooks.3+`)
    /// ignores a veto returned from them.

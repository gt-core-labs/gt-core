...
async fn harness(
    test: &str,
) -> Option<(DocumentsHandler, PgDocumentsResource, Arc<WsPools>, sqlx::PgPool, TempDir)> {
    ...
    let handler =
        DocumentsHandler::new(pools.clone(), Some(blob), "test-bucket", Extractor::without_ocr(), None);
...
async fn call(
    ...
    h.dispatch(tool, DomainCtx { workspace: ws, actor: "doc-test", args }).await
}
...
async fn documents_tool_lifecycle_validate_and_execute() {
    ...
    call(&h, None, "documents.attach.validate", json!({
    ...
    let created = call(&h, None, "documents.attach.execute", json!({
        "owner_type": "epic", "owner_id": owner, "kind": "md",
    ...
    assert!(
        call(&h, None, "documents.update.validate", json!({
    ...
    let updated = call(&h, None, "documents.update.execute", json!({
    ...
    call(&h, None, "documents.remove.execute", json!({ "id": id, "expected_version": 1 }))
        .await
        .expect("remove executes");
    let after = call(&h, None, "documents.list.execute", json!({
        "owner_type": "epic", "owner_id": owner
    }))
    .await
    .unwrap();
    assert!(
        !after["documents"].as_array().unwrap().iter().any(|d| d["id"] == id),
...
async fn attach_blob_uploads_and_extracts_text() {
    let Some((h, _res, _pools, _admin, dir)) = harness("attach_blob_uploads_and_extracts_text").await
    else {
        return;
    };
    let n = nonce();
    ...
    assert_eq!(created["bucket"], "test-bucket", "blob row records the configured bucket");
    let key = created["key"].as_str().expect("blob key").to_string();
    assert!(key.contains("/epic/"), "key is per-owner content-addressed: {key}");
    assert!(
        created["extracted_text"].as_str().unwrap_or("").contains("objectives"),
    ...
    assert!(on_disk.exists(), "blob object written to the store at {key}");
}
...
async fn resource_reads_resolve_doc_and_owner_inline() {
    ...
    let one = res.get(None, &id).await.expect("resource get").expect("doc present");
    assert_eq!(one["id"], created["id"]);
    assert_eq!(one["body_md"], "inline me");

    // gt://issue/{id} inline — the owner's live documents.
    let inline = res.list_for_owner(None, &owner).await.expect("list_for_owner");
    assert!(inline.iter().any(|d| d["id"] == created["id"]), "owner inline carries the doc");

    // An absent id resolves to None (not-found).
    assert!(
        res.get(None, &format!("missing-{n}")).await.expect("resource get").is_none(),
...
async fn documents_are_isolated_per_workspace() {
    ...
    let owner = format!("epic-{n}");
    let created = call(&h, Some(&ws_a), "documents.attach.execute", json!({
        "owner_type": "epic", "owner_id": owner, "kind": "blob",
    ...
    assert!(key.starts_with(&format!("{ws_a}/")), "blob key namespaced under the tenant: {key}");

    // Workspace A sees it; workspace B does not (separate schema).
    let in_a = call(&h, Some(&ws_a), "documents.list.execute", json!({
        "owner_type": "epic", "owner_id": owner
    }))
    .await
    .unwrap();
    assert!(in_a["documents"].as_array().unwrap().iter().any(|d| d["id"] == created["id"]));

    let in_b = call(&h, Some(&ws_b), "documents.list.execute", json!({
        "owner_type": "epic", "owner_id": owner
    }))
    .await
    .unwrap();
    ...
    assert!(res.get(Some(&ws_a), id).await.unwrap().is_some(), "doc visible in its own tenant");
    assert!(res.get(Some(&ws_b), id).await.unwrap().is_none(), "doc invisible in another tenant");
}

...
fn parity_map(ns: &str) -> Vec<Route> {
    match ns {
        ...
        "documents" => vec![
            ...
            rt("GET", "/{id}/download", None), // Raw document bytes download — REST-only streaming endpoint
...
fn is_routable(tool: &str) -> bool {
    !tool.ends_with(".validate")
        ...
        // embedding index — no REST equivalent; streaming bytes use GET /{id}/download instead.

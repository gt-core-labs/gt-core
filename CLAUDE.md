...
- Migration from gastown to gt-core is crate-by-crate (see [docs/01-migration-plan.md](docs/01-migration-plan.md)). The P4 kernel freeze LIFTED 2026-06-02 (`hq-mod-refactor.10..12` closed replay-green): `gt-events` is PORTED UP (`hq-core-port.10`); `gt-bus`/`gt-plugin`/`gt-telemetry` may now follow. Gastown's event-replay `gt-audit` (EventStore) still migrates up under a rename (collides with gt-core's RBAC-audit `gt-audit`).
...
- **Re-inventing kernel primitives** that already exist in gastown (gt-bus, gt-plugin, gt-telemetry; `gt-events` now lives in `crates/kernel/gt-events`). They migrate up in P4 — don't fork them.
